...
The dev proxy targets QuicProxy API at `http://127.0.0.1:1235` by default. Override it with:

```bash
QUICPROXY_API_TARGET=http://127.0.0.1:1235 npm run dev -- --host 0.0.0.0
```

In the Quicboard Settings page, leaving `API base URL` empty uses the same-origin Vite proxy. This avoids browser CORS issues when developing locally.
...
- If adding a new Quicboard API call, update both `quicboard/src/lib/api.ts` and the proxy route list in `quicboard/vite.config.ts`.
